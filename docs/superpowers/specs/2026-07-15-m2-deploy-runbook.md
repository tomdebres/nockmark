...
```bash
cd "$HOME"
git clone --filter=blob:none https://github.com/zorp-corp/nockchain nockchain
...
### Step 5: Compile the registry binary
...
### Step 6: Verify roswell.jam is present before building

Ensure `tock/assets/roswell.jam` exists before `cargo build --release` in Step 5. If you have miner.jam but not roswell.jam:
...
2. **Run tock to prove k=2 proofs against the nonce:**
   ...
   # Boot the miner kernel once (serf persists for all proofs)
   cd "$HOME/nockmark/m0-prover-spike"
   ./target/release/tock prove --nonce "$NONCE/0" --kernel tock/assets/miner.jam --out proof0.jam
   ./target/release/tock prove --nonce "$NONCE/1" --kernel tock/assets/miner.jam --out proof1.jam
   ```
...
**Proof verification fails:** Check that the verifier received the correct kernel bytes (registry.jam hash should match build-time compile-in). Kernel crashes usually show as `SEGV` in journalctl.
